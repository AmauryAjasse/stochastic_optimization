...
def repl_bat_rule(b, discount_rate=1, replacement_year=(10,)):
...
def expected_fuel_cost_rule(b, s, discount_rate=1, total_duration=20):
    """Coût du carburant consommé. C'est le seul coût qui dépend de la gestion d'énergie et qui est donc propre à
    chaque scénario."""
    expected_fuel = sum(b.gen[s].e_th[t] * b.gen[s].fuel_cost * b.gen[s].fuel_consumption for t in b.time)
    return expected_fuel * sum(1 / (1 + discount_rate) ** i for i in range(0, total_duration))

def total_cost_rule(b, s, with_diesel_generator=0, discount_rate=1, total_duration=20, replacement_year=[5, 10, 15]):
    if with_diesel_generator == 0:
        ...
        repl = repl_bat_rule(b, discount_rate, replacement_year)
        fuel = expected_fuel_cost_rule(b, s, discount_rate, total_duration)
